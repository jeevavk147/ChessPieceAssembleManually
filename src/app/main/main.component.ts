...
export class MainComponent implements OnInit, OnChanges{
  ...
  async  currentAuthenticatedUser() {
    ...
    this.userId=userId
    this.boardManager.engineFacade.userId=userId
    this.username=username
    this.boardManager.engineFacade.user1=username
    this.api.createItem({userid:userId,username:username}).subscribe(
          {
              next:()=>{
                    this.api.getitem(userId).subscribe((item)=>{
                      this.usercolor=item.item.color
                      this.boardManager.engineFacade.usercolor=item.item.color
